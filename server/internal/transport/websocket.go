...
// WSConn реализует интерфейс Conn поверх gorilla/websocket.
type WSConn struct {
	conn *websocket.Conn
}

// UpgradeHTTP выполняет WebSocket upgrade для HTTP-запроса.
...
// DialWS подключается к WebSocket-серверу (клиентская сторона).
