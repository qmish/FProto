...
func performHandshake(t *testing.T) (*NoiseSession, *NoiseSession) {
	...
	// Каналы для передачи handshake-сообщений между клиентом и сервером
...
func TestEncryptDecryptClientToServer(t *testing.T) {
	...
	plaintext := []byte("Привет от клиента!")
...
func TestEncryptDecryptServerToClient(t *testing.T) {
	...
	plaintext := []byte("Привет от сервера!")
...
func TestBidirectionalCommunication(t *testing.T) {
	...
	messages := []struct {
		...
		{"client", "Сообщение 1 от клиента"},
		{"server", "Ответ 1 от сервера"},
		{"client", "Сообщение 2 от клиента"},
		{"server", "Ответ 2 от сервера"},
		{"client", "Сообщение 3 от клиента"},
	}
...
func TestDecryptTamperedCiphertext(t *testing.T) {
	...
	plaintext := []byte("целостность данных")
	ciphertext, err := client.Encrypt(plaintext)
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
