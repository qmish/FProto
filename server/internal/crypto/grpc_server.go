...
// CryptoGRPCServer implements the CryptoService gRPC API.
...
func NewCryptoGRPCServer(serverKey *KeyPair) *CryptoGRPCServer {
...
func (s *CryptoGRPCServer) InitiateHandshake(_ context.Context, req *pb.HandshakeInitRequest) (*pb.HandshakeInitResponse, error) {
...
func (s *CryptoGRPCServer) ProcessHandshake(_ context.Context, req *pb.HandshakeStepRequest) (*pb.HandshakeStepResponse, error) {
...
func (s *CryptoGRPCServer) Encrypt(_ context.Context, req *pb.EncryptRequest) (*pb.EncryptResponse, error) {
	aead, err := NewAEAD(req.CryptoState[:32])
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "invalid crypto state: %v", err)
	}

	ciphertext := EncryptAEAD(aead, req.SessionId, req.Seq, req.Plaintext)
...
func (s *CryptoGRPCServer) Decrypt(_ context.Context, req *pb.DecryptRequest) (*pb.DecryptResponse, error) {
	aead, err := NewAEAD(req.CryptoState[:32])
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "invalid crypto state: %v", err)
	}

	plaintext, err := DecryptAEAD(aead, req.SessionId, req.Seq, req.Ciphertext)
	if err != nil {
...
func (s *CryptoGRPCServer) Rekey(_ context.Context, req *pb.RekeyRequest) (*pb.RekeyResponse, error) {
	if len(req.CryptoState) < 32 {
		return nil, status.Errorf(codes.InvalidArgument, "crypto state too short")
	}

	newKey, err := Rekey(req.CryptoState[:32])
	if err != nil {
